...
public class FileUtil {
	...
	public static void copyDir( File from, File to ) throws IOException {
		...
		if ( from.isFile() ) {
		  ...
		  if (to.isFile()) throw new IOException("cannot copy directory \"" + from + "\" into the file" + to + "\".");
			to.mkdirs();
			String[] childs = from.list();
			if ( childs != null ) {
				for ( int i = 0; i < childs.length; i++ ) {
					File childFrom = new File( from.getAbsolutePath() + File.separator + childs[i] );
					File childTo = new File( to.getAbsolutePath() + File.separator + childs[i] );
	...
	/**
	 * Utility method to get extension of a file
	 * return empty String if @file doesn't exist or if @file doesn't have extension
	 */
	...
	/**
	 * Utility method to get extension of a file
	 * return empty String if @file doesn't have extension
	 */
